  ...
  const load = async () => {
    ...
    if (tab === 'collections') {
      ...
      out.innerHTML = kpis([[t('total'), usd(tot)], [t('count'), rows.length], [t('k_deposit'), usd(rows.filter((r) => r.kind === 'deposit').reduce((a, r) => a + r.amount_usd, 0))], [t('k_payment'), usd(rows.filter((r) => r.kind === 'payment').reduce((a, r) => a + r.amount_usd, 0))]]) + `
        ...
        ${rows.length ? `<div class="table-wrap"><table class="t"><thead><tr><th>${esc(t('receipt'))}</th><th>${esc(t('date'))}</th><th>${esc(t('shipment'))}</th><th>${esc(t('customer'))}</th><th>${esc(t('method'))}</th><th class="num">${esc(t('amount'))}</th><th class="num">USD</th><th>${esc(t('by'))}</th></tr></thead>
        <tbody>${rows.map((r) => `<tr><td class="mono">${esc(r.ref)}</td><td class="nowrap">${fdate(r.received_at)}</td><td class="mono"><a href="#/shipment/${r.shipment_id}">${esc(r.shipment_ref)}</a></td><td>${esc(r.customer_name)}</td>
      ...
      lastCSV = ['collections', toCSV(rows, [{ label: 'Receipt', key: 'ref' }, { label: 'Date', value: (r) => r.received_at.slice(0, 10) }, { label: 'Kind', key: 'kind' }, { label: 'Shipment', key: 'shipment_ref' },
    ...
    if (tab === 'receivables') {
      const rows = await run(from('v_shipments').select('*').gt('balance_usd', 0).neq('status', 'cancelled').order('balance_usd', { ascending: false }).limit(5000));
      ...
      out.innerHTML = kpis([[t('outstanding'), usd(tot)], ['0–30', usd(buckets['0–30'])], ['31–90', usd(buckets['31–60'] + buckets['61–90'])], ['90+', usd(buckets['90+'])]]) + (rows.length ? `
        <div class="table-wrap"><table class="t"><thead><tr><th>${esc(t('shipment'))}</th><th>${esc(t('customer'))}</th><th>${esc(t('status'))}</th><th class="num">${esc(t('invoice_total'))}</th><th class="num">${esc(t('paid'))}</th><th class="num">${esc(t('balance'))}</th><th class="num">Days</th></tr></thead>
        <tbody>${rows.map((r) => `<tr class="click" data-href="#/shipment/${r.id}"><td class="mono">${esc(r.ref)}</td><td>${esc(r.customer_name)}<div class="muted small">${esc(r.customer_phone)}</div></td><td>${statusBadge(r.status)}</td>
      ...
      lastCSV = ['receivables', toCSV(rows, [{ label: 'Shipment', key: 'ref' }, { label: 'Customer', key: 'customer_name' }, { label: 'Phone', key: 'customer_phone' }, { label: 'Status', key: 'status' },
    ...
    if (tab === 'revenue') {
      ...
      lastCSV = ['revenue', toCSV(rows, [{ label: 'Invoice', key: 'invoice_ref' }, { label: 'Issued', value: (r) => r.issued_at.slice(0, 10) }, { label: 'Shipment', key: 'shipment_ref' }, { label: 'Customer', key: 'customer_name' },
    ...
    if (tab === 'volumes') {
      const rows = await run(from('v_shipments').select('*').gte('created_at', d1).lt('created_at', d2).neq('status', 'cancelled').limit(10000));
      const key = (r) => `${r.mode.toUpperCase()} ${r.origin_branch}→${r.destination_branch}`;
      const agg = {};
      rows.forEach((r) => { const k = key(r); agg[k] = agg[k] || { n: 0, cbm: 0, kg: 0, inv: 0 }; agg[k].n++; agg[k].cbm += Number(r.cbm ?? 0); agg[k].kg += Number(r.weight_kg ?? r.actual_kg ?? 0); agg[k].inv += Number(r.invoice_total || 0); });
      const tcbm = rows.reduce((a, r) => a + Number(r.cbm ?? 0), 0);
      out.innerHTML = kpis([[t('shipments'), rows.length], ['CBM', num(tcbm, 2)], ['kg', num(rows.reduce((a, r) => a + Number(r.weight_kg ?? r.actual_kg ?? 0), 0), 0)], [t('invoice_total'), usd(rows.reduce((a, r) => a + Number(r.invoice_total || 0), 0))]]) + `
        <div class="table-wrap"><table class="t"><thead><tr><th>${esc(t('route'))}</th><th class="num">${esc(t('shipments'))}</th><th class="num">CBM</th><th class="num">kg</th><th class="num">${esc(t('invoice_total'))}</th><th class="num">USD / CBM</th></tr></thead>
