...
import { icon, esc, usd, num, fdate, modeTag, route, shipBadge, modal, empty, $, $$, toCSV, downloadCSV } from '../ui.js';
import { companySelect, getCompany, setCompany, accName, signed } from '../acc.js';

const TABS = ['pl', 'bs', 'tb', 'containers'];
...
export async function render({ el, setTitle, query }) {
  ...
  async function containers(d1, d2) {
    const co = getCompany();
    ...
    out.innerHTML = `<div class="card-b"><div class="grid c4">
      ...
      <p class="muted small" style="margin:10px 0 0">${esc(t('acc_container_hint'))}</p></div>
      ${rows.length ? `<div class="table-wrap"><table class="t"><thead><tr><th>${esc(t('container'))}</th><th class="hide-m">${esc(t('status'))}</th><th class="num hide-m">${esc(t('bookings'))}</th><th class="num hide-m">CBM</th>
        <th class="num">${esc(t('acc_revenue'))}</th><th class="num">${esc(t('acc_cost'))}</th><th class="num">${esc(t('acc_profit'))}</th><th class="num hide-m">%</th><th class="num hide-m">/CBM</th></tr></thead>
        <tbody>${rows.map((r) => `<tr class="click" data-ship="${r.id}"><td>${modeTag(r.mode)} <b class="mono">${esc(r.container_no || r.ref)}</b><div class="muted small">${esc(r.company)} · ${esc(r.origin)}→${esc(r.destination)} · ${fdate(r.departed_at)}</div></td>
          <td class="hide-m">${shipBadge(r.status)}</td><td class="num hide-m">${r.bookings}</td><td class="num hide-m">${num(r.cbm, 2)}</td>
          ...
          <td class="num hide-m">${r.margin_pct ?? '—'}</td><td class="num hide-m">${r.profit_per_cbm ?? '—'}</td></tr>`).join('')}</tbody></table></div>` : empty(t('nothing_here'), 'container')}`;
    csv = () => downloadCSV(`horse-cargo-container-pnl-${d1}-${d2}.csv`, toCSV(rows, [
      { label: 'Shipment', key: 'ref' }, { label: 'Container', key: 'container_no' }, { label: 'Mode', key: 'mode' }, { label: 'Company', key: 'company' },
      { label: 'Origin', key: 'origin' }, { label: 'Destination', key: 'destination' }, { label: 'Status', key: 'status' }, { label: 'Departed', key: 'departed_at' },
      { label: 'Bookings', key: 'bookings' }, { label: 'CBM', key: 'cbm' }, { label: 'Revenue USD', key: 'revenue' }, { label: 'Cost USD', key: 'cost' },
      { label: 'Profit USD', key: 'profit' }, { label: 'Margin %', key: 'margin_pct' }, { label: 'Profit per CBM', key: 'profit_per_cbm' }]));
  }
...
export function pnlBody(p) {
  const profit = p.revenue - p.direct_cost - p.shared_cost;
  ...
  return `<div class="money" style="margin-bottom:12px">
      ...
      <div><div class="k">${esc(t('acc_container_costs'))}</div><div class="v">${usd(p.shared_cost)}</div></div>
      <div><div class="k">${esc(t('acc_direct_costs'))}</div><div class="v">${usd(p.direct_cost)}</div></div>
      <div><div class="k">${esc(t('acc_profit'))}</div><div class="v ${profit < 0 ? 'due' : 'ok'}">${usd(profit)}</div></div></div>
    ${p.unallocated_cost ? `<div class="callout warn" style="margin-bottom:10px">${icon('alert')}<div>${esc(t('acc_unallocated'))}: ${usd(p.unallocated_cost)}</div></div>` : ''}
    ${p.bookings.length ? `<div class="table-wrap"><table class="t"><thead><tr><th>${esc(t('booking'))}</th><th class="num">${unit}</th><th class="num">${esc(t('acc_revenue'))}</th><th class="num">${esc(t('acc_cost'))}</th><th class="num">${esc(t('acc_profit'))}</th></tr></thead>
      <tbody>${p.bookings.map((b) => `<tr><td><a class="mono" href="#/booking/${b.id}">${esc(b.ref)}</a><div class="muted small">${esc(b.customer)}</div></td>
        <td class="num">${num(p.shipment.mode === 'air' ? b.chargeable_kg : b.cbm, 2)}</td><td class="num">${usd(b.revenue, { bare: true })}</td>
        <td class="num">${usd(b.direct_cost + b.allocated_cost, { bare: true })}</td><td class="num" style="font-weight:700">${amt(b.profit)}</td></tr>`).join('')}</tbody></table></div>` : ''}
    <h3 style="margin:16px 0 6px;font-size:14px">${esc(t('acc_cost_lines'))}</h3>
    ${p.cost_lines.length ? `<div class="table-wrap"><table class="t"><tbody>${p.cost_lines.map((c) => `<tr><td class="nowrap small">${fdate(c.date)}</td>
      <td><span class="mono small">${esc(c.code)}</span> ${esc(c.account)}<div class="muted small">${esc(c.description || '')}${c.booking_ref ? ' · ' + esc(c.booking_ref) : ''} · ${esc(c.journal)}</div></td>
      <td class="num">${amt(c.amount)}</td></tr>`).join('')}</tbody></table></div>` : `<p class="muted small">${esc(t('acc_no_costs'))}</p>`}`;
}
...
export async function shipmentPnlModal(id) {
  try {
    ...
    modal({
      title: `${t('acc_container_pnl')} · ${p.shipment.container_no || p.shipment.ref}`, wide: true, body: pnlBody(p),
      foot: `<button class="btn" data-close>${esc(t('close'))}</button>${can('acc.write') ? `<a class="btn" data-close href="#/acc/expenses?new=1&shipment=${p.shipment.id}">${icon('plus')}${esc(t('acc_new_expense'))}</a><a class="btn primary" data-close href="#/acc/bills/new?shipment=${p.shipment.id}">${icon('plus')}${esc(t('acc_add_cost'))}</a>` : ''}
        <a class="btn" data-close href="#/shipment/${p.shipment.id}">${icon('container')}${esc(t('open'))}</a>`,
    });
